...
mod tests {
    ...
    async fn when_valid_link_is_passed_should_retrieve_info_and_store() {
        ...
        let result = http_client.post(format!("{}links", api_endpoint))
        ...
        let response_data: ShortUrl = serde_json::from_str(response.text().await.unwrap().as_str()).unwrap();
        
        assert_eq!(response_data.original_link, "https://google.com");
        
        let redirect_response = http_client.get(format!("{}{}", api_endpoint, response_data.link_id))
            .send()
            .await
            .unwrap();
    ...
    async fn retrieve_api_endpoint() -> String {
        ...
        let get_stacks = cloudformation_client.describe_stacks()

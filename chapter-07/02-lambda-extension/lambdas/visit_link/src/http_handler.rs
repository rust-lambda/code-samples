...
pub(crate) async fn function_handler<R: UrlRepository, I: UrlInfo, T: Config>(
    ...
    let full_url = url_shortener
        .retrieve_url_and_increment_clicks(&config, link_id)
        .await;
...
mod tests {
    ...
    async fn when_valid_request_made_with_path_parameter_should_return_redirect() {
        ...
        mock_url_repo
            ...
            .times(1)
            .with(predicate::always(), predicate::eq("aoinf87".to_string())) // make sure the correct id is propagated
            .returning(|_, _link_id| Ok(Some("https://google.com".to_string())));
    ...
    async fn when_database_errors_should_return_500() {
        ...
        mock_url_repo
            .expect_get_url_from_short_link()
            .times(1)
            .with(predicate::always(), predicate::eq("aoinf87".to_string()))
            .returning(|_, _link_id| Err("Failed to retrieve from DB".to_string()));
        let url_shortener = UrlShortener::new(mock_url_repo, mock_url_info);
        let mut path_params = HashMap::new();
        path_params.insert("linkId".to_string(), "aoinf87".to_string());
        let request = Request::builder()

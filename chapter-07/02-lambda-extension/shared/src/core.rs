...
impl<R: UrlRepository, I: UrlInfo> UrlShortener<R, I> {
    ...
    pub async fn retrieve_url_and_increment_clicks(
        ...
        self.url_repo
            .get_url_from_short_link(configuration, link_id)
            .await
    }
    ...
    pub async fn list_urls(
        ...
        let (short_urls, last_evaluated_id) = self
            .url_repo
            .list_urls(configuration, last_evaluated_id)
            .await?;
...
mod tests {
    ...
    async fn when_valid_link_is_passed_should_retrieve_info_and_store() {
        ...
        url_repo
            ...
            .times(1)
            .returning(|_, url_to_shorten, short_url, url_details| {
                Ok(ShortUrl::new(
                    short_url,
                    url_to_shorten,
                    0,
    ...
    async fn on_valid_call_should_search_and_return() {
        ...
        url_repo
            ...
            .with(
                predicate::always(),
                predicate::eq(test_short_url.to_string()),
            )
            .times(1)
            .returning(|_, _url_to_shorten| Ok(Some("https://google.com".to_string())));
        ...
        let result = url_shortener
            .retrieve_url_and_increment_clicks(&configuration, test_short_url)
            .await;
    ...
    async fn on_url_not_found_call_should_return_error() {
        ...
        url_repo
            ...
            .times(1)
            .returning(|_, _url_to_shorten| Err("Not found".to_string()));
    ...
    async fn on_list_urls_should_return_vec() {
        ...
        let result = url_shortener.list_urls(&configuration, None).await;
    ...
    async fn on_list_urls_error_should_pass_up() {
        ...
        url_repo
            ...
            .with(predicate::always(), predicate::eq(None))
            .times(1)
            .returning(|_, _url_to_shorten| Err("Error".to_string()));
